...
df10 = pd.read_csv('/home/drugo/sim_prospero_5_sites/data/'+def_site+'/'+def_site+'_PRP.csv', na_values=-9999, parse_dates=[1], index_col=[1], skiprows=2)
...
df10 = df10.iloc[:,1:].reindex(index)

df = pd.concat([df,df3,df6,df9,df10],axis=1)
df.columns = ['obs','ps','fao','pt','prp']
print(df.head())
...
dfMonth = dfDay[['obs','ps','fao','pt','prp']].copy()
dfYear = dfDay[['obs','ps','fao','pt','prp']].copy()
...
dfMonth.plot(y=['obs','ps','fao','pt'],
             kind='line', ax = ax1, 
             colormap='Set2',linewidth=5, rot = -0,stacked=False,
             label = ['Observed','PS','FAO','PT'])
...
dfYear.plot(y=['obs','ps','fao','pt','prp'],
            x='year',kind='bar', ax = ax2, 
             colormap='Set2',edgecolor=".3",linewidth=1.5, rot = -0,stacked=False,
             label = ['Observed','PS','FAO','PT','Precipitation'])
